...
var handleEventToSyncDB = function handleEventToSyncDB(event, client, flush) {
  ...
  if (type === 'message.new') {
    ...
    if (message && (!message.parent_id || message.show_in_channel)) {
      return queriesWithChannelGuard(function (flushOverride) {
        var queries = (0, _upsertMessages.upsertMessages)({
          flush: flushOverride,
          messages: [message]
        });
        if (_cid && client.user && client.user.id !== (_user == null ? void 0 : _user.id)) {
          var userId = client.user.id;
          var channel = client.activeChannels[_cid];
          if (channel) {
            var ownReads = channel.state.read[userId];
            var unreadCount = channel.countUnread();
            var upsertReadsQueries = (0, _upsertReads.upsertReads)({
              cid: _cid,
              flush: flushOverride,
              reads: [{
